...
public:
  explicit exchange(const exchange &other)
      : conn_(other.conn_), queue_(NULL), id_(other.id_ == rap_conn_exchange_id ? conn_.next_id() : other.id_), send_window_(other.send_window_)
  {
    start_write();
  }

  explicit exchange(const exchange_t &other)
      : conn_(other.conn()), queue_(NULL), id_(other.id() == rap_conn_exchange_id ? conn_.next_id() : other.id()), send_window_(other.send_window())
  {
    start_write();
  }

  explicit exchange(conn_t &conn)
      : conn_(conn), queue_(NULL), id_(rap_conn_exchange_id), send_window_(conn.send_window())
  {
    start_write();
  }
  ...
  error process_frame(const frame *f)
  {
    ...
    if (f->header().has_head())
    {
      ...
      if (error e = self().process_head(r))
        return e;
    }
    if (f->header().has_body())
    {
      if (error e = self().process_body(r))
        return e;
    }
    if (f->header().is_final())
    {
      if (error e = self().process_final(r))
        return e;
    }
    return rap_err_ok;
  }
