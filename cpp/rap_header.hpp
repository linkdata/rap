...
public:
  ...
  void set_size_value(size_t n) { buf_[0] = static_cast<unsigned char>(n >> 8); buf_[1] = static_cast<unsigned char>(n); }

  uint16_t id() const { return static_cast<uint16_t>(buf_[2] & mask_id) << 8 | buf_[3]; }
  void set_id(uint16_t id) { buf_[2] = static_cast<unsigned char>(id >> 8) & mask_id; buf_[3] = static_cast<unsigned char>(id); }
