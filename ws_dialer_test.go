...
func newWsDialer(h http.Handler) *websocket.Dialer {
	client, server, stopfn, _ := makeConnPipe()
	conn := &recorder{server: server, stopfn: stopfn}
...
func (r *recorder) Hijack() (net.Conn, *bufio.ReadWriter, error) {
	if exch, ok := r.server.(*Exchange); !ok {
		panic("can't hijack the conn; not an exchange")
	} else {
		return exch.Hijack()
	}
}
